...
may be in a position to cope resource-wise with the healthcare consequences
...
employed in the informal sector \parencite{Aguila2011}.}, possibly limiting their access to quality healthcare and hence
...
alive any more or were living in a non-surveyed household at the time
...
\end{equation}
In equation. \ref{eq:cha3_employed-1}, $Diabetes_{i}$
is a dummy variable and is modelled as a function of the same socioeconomic
and demographic factors $X_{i}$ as in equation \ref{eq:cha3_employed-2}
...
equation \ref{eq:cha3_employed} is identical to the probit specification
(equation \ref{eq:cha3_employed-2}) and estimates the effect of diabetes
...
probit models \parencite{Knapp1998}. If $u_{i}$ 
 and $\eta_{i}$ are correlated, the estimation of equation \ref{eq:cha3_employed-2}
...
\ref{eq:cha3_employed-1-1}) and a second stage (equation \ref{eq:cha3_employed-3}).
...
are the same as in the bivariate probit case described in equation
\ref{eq:cha3_employed-1} and equation \ref{eq:cha3_employed}. In the linear \ac{IV}
...
correlation between the disturbance terms of equation \ref{eq:cha3_employed-1}
and equation \ref{eq:cha3_employed} for males and females, suggesting exogeneity
...
and strong effect is only found in the poorest quartile, where employment
...
defined as working without a written contract, being self-employed or working in semi-subsistence agriculture. 
...
is rather counter-intuitive because there is no obvious reason why
...
Why was diabetes found to be exogenous in the Mexican
...
this particular dataset as it provided us with a sensible instrument
...
after diagnosis for males, whereas for females effects appear
...
the prevention and treatment of diabetes for the poor, especially if
